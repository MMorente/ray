...
from ci.ray_ci.docker import run_command, run_tests
...
def test_run_tests() -> None:
    ...
    with mock.patch(
        "ci.ray_ci.docker._run_tests_in_docker",
        side_effect=_mock_run_tests_in_docker,
    ):
        # test_targets and commands are not empty
        assert run_tests(["t1", "t2"], ["some_command"], 2) == 0
        # command is empty
        assert run_tests(["t1", "t2"], [], 2) == 1
        # test_targets is empty after chunking
        assert run_tests(["t1"], ["some_command"], 2) == 1
